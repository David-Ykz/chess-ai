...
INCBIN(EVAL, "nets/hexadecane_512_v2.net");
...
template void Net::updateAccumulator<true>(PieceType, Color, Square, Square, Square);
template void Net::updateAccumulator<false>(PieceType, Color, Square, Square, Square);

template <bool add>
void Net::updateAccumulator(PieceType pieceType, Color pieceColor, Square square,
                            Square kingSquare_White, Square kingSquare_Black) {
    ...
    for (auto side : {Color::WHITE, Color::BLACK}) {
        const int inputs =
            index(pieceType, pieceColor, square, side, side == Color::WHITE ? kingSquare_White : kingSquare_Black);
...
void Net::updateAccumulator(PieceType pieceType, Color pieceColor, Square from_square, Square to_square, Square kingSquare_White, Square kingSquare_Black) {
    Accumulator &accumulator = accumulator_stack[currentAccumulator];
#if defined(USE_SIMD)
    for (auto side : {Color::WHITE, Color::BLACK}) {
        const int inputClear =
            index(pieceType, pieceColor, from_square, side, side == Color::WHITE ? kingSquare_White : kingSquare_Black);
        const int inputAdd =
            index(pieceType, pieceColor, to_square, side, side == Color::WHITE ? kingSquare_White : kingSquare_Black);
...
void Net::refresh(Board &board) {
    ...
    Bitboard pieces = board.all();

    const Square kingSquare_White = board.kingSq(Color::WHITE);
    const Square kingSquare_Black = board.kingSq(Color::BLACK);

    while (pieces) {
        Square sq = pieces.pop();
        PieceType pt = board.at<PieceType>(sq);

        updateAccumulator<true>(pt, board.at(sq).color(), sq, kingSquare_White, kingSquare_Black);
    }
}
